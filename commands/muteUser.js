    ...
    execute(message, args, H4r1ey, Discord) {
        ...
        if (message.member.roles.cache.some(h => per.includes(h.id))) {
            ...
            const embed4 = new Discord.MessageEmbed()
                .setDescription(`<@${message.author.id}> You cannot mute yourself!`)
                .setColor(H4r1ey.config.customConfig.hexID)
            ...
            const embed5 = new Discord.MessageEmbed()
                .setDescription(`<@${message.author.id}> You cannot mute the owner of the server!`)
                .setColor(H4r1ey.config.customConfig.hexID)
            ...
            const embed44 = new Discord.MessageEmbed()
                .setDescription(`<@${message.author.id}> You cannot mute the bot!`)
                .setColor(H4r1ey.config.customConfig.hexID)
            ...
            user.roles.add(mutedRoleIDD)
            if(H4r1ey.config.optionalSettings.removeMemberRoleOnMute) {
                user.roles.remove(H4r1ey.config.optionalSettings.memberRoleID)
            }
            console.log(chalk.blue(`${user.user.tag} has been muted by ${message.author.tag} for ${warnReason}`))


            if(H4r1ey.config.optionalSettings.DMUsersOnPunishment) {
                const embed0000 = new Discord.MessageEmbed()
                .setDescription(`You have been muted by \`${message.author.tag}\` for \`${warnReason}\` in \`${message.guild.name}\``)
                .setColor(H4r1ey.config.customConfig.hexID)
                .setFooter("©️ " + H4r1ey.config.customConfig.serverCopyright, H4r1ey.config.customConfig.serverIcon)
            ...
            if (H4r1ey.config.optionalSettings.logMutes) {
                ...
                if (!channel2) return console.log(chalk.red(`The channel ID set for the mute command is in-correct. Please re-config!`))
