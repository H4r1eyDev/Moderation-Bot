...
module.exports = {
    ...
    execute(message, args, H4r1ey, Discord) {
        ...
        if (H4r1ey.config.optionalSettings.logCommands) {
            ...
            if (!channel) return console.log(chalk.red(`The channel ID set for the kick command is in-correct. Please re-config!`))
        ...
        const per = H4r1ey.config.permissionSetup.kickCommandPerms
        if (message.member.roles.cache.some(h => per.includes(h.id))) {
            ...
            const embed5 = new Discord.MessageEmbed()
                .setDescription(`<@${message.author.id}> You cannot kick the owner of the server!`)
                .setColor(H4r1ey.config.customConfig.hexID)
            ...
            .setDescription(`<@${message.author.id}> You cannot kick the bot!`)
            .setColor(H4r1ey.config.customConfig.hexID)
            ...
            console.log(chalk.blue(`${user.user.tag} has been kicked by ${message.author.tag} for ${warnReason}`))

            user.kick();

            if(H4r1ey.config.optionalSettings.DMUsersOnPunishment) {
                const embed0000 = new Discord.MessageEmbed()
                .setDescription(`You have been kicked by \`${message.author.tag}\` for \`${warnReason}\` in \`${message.guild.name}\``)
                .setColor(H4r1ey.config.customConfig.hexID)
                .setFooter("©️ " + H4r1ey.config.customConfig.serverCopyright, H4r1ey.config.customConfig.serverIcon)
            ...
            if (H4r1ey.config.optionalSettings.logKicks) {
                const channel2 = H4r1ey.channels.cache.get(H4r1ey.config.loggingChannels.kickLoggingChannel);
                if (!channel2) return console.log(chalk.red(`The channel ID set for the warn command is in-correct. Please re-config!`))
