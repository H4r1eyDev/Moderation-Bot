...
module.exports = {
    ...
    execute(message, args, H4r1ey, Discord) {
        ...
        if (H4r1ey.config.optionalSettings.logCommands) {
            ...
            if (!channel) return console.log(chalk.red(`The channel ID set for the warn command is in-correct. Please re-config!`))
        ...
        const per = H4r1ey.config.permissionSetup.warnCommandPerms
        if (message.member.roles.cache.some(h => per.includes(h.id))) {
            ...
            const embed4 = new Discord.MessageEmbed()
                .setDescription(`<@${message.author.id}> You cannot warn yourself!`)
                .setColor(H4r1ey.config.customConfig.hexID)
            ...
            const embed5 = new Discord.MessageEmbed()
                .setDescription(`<@${message.author.id}> You cannot warn the owner of the server!`)
                .setColor(H4r1ey.config.customConfig.hexID)
            ...
            .setDescription(`<@${message.author.id}> You cannot warn the bot!`)
            .setColor(H4r1ey.config.customConfig.hexID)
            ...
            console.log(chalk.blue(`${user.user.tag} has been warned by ${message.author.tag} for ${warnReason}`))

            if(H4r1ey.config.optionalSettings.DMUsersOnPunishment) {
                const embed0000 = new Discord.MessageEmbed()
                .setDescription(`You have been warned by \`${message.author.tag}\` for \`${warnReason}\` in \`${message.guild.name}\``)
                .setColor(H4r1ey.config.customConfig.hexID)
                .setFooter("©️ " + H4r1ey.config.customConfig.serverCopyright, H4r1ey.config.customConfig.serverIcon)
            ...
            if (H4r1ey.config.optionalSettings.logWarnings) {
